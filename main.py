...
def main():
    ...
    print("Creating ComponentA (sender), ComponentB (receiver with forward), and ComponentC (listener with feedback)...")
    
    # Create ComponentA with connection_out for sending and connection_feedback for receiving feedback from C
    component_a = ComponentA(connection_out=connection, connection_feedback=feedback_connection)
    
    # Create ComponentB with connection_in for receiving and connection_forward for forwarding
    component_b = ComponentB(connection_in=connection, connection_forward=forward_connection)
    
    # Create ComponentC with connection_forward for listening to forwarded data and connection_feedback for sending feedback
    component_c = ComponentC(connection_forward=forward_connection, connection_feedback=feedback_connection)
